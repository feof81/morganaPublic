...
int main(int argc, char *argv[])
{
  ...
  Teuchos::RCP<Epetra_MultiVector> V(new Epetra_MultiVector(rowMapA,1));
  ...
  preconditionerList->set("fact: level-of-fill"  , 6);
  preconditionerList->set("overlap"              , 4);
  ...
  epetraOperatorPrecPrjA op;
  op.setVector(b,V);
  op.setPreconditioner(belosPrec);
  op.Apply(*X,*Y);
