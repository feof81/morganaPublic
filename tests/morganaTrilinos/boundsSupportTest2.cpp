...
int main(int argc, char *argv[])
{
  ...
  //Matrix matrixAA------------------------------------------------------------
  Epetra_CrsMatrix matrixAA(Copy, rowMapA, 1);
  ...
  Epetra_CrsMatrix matrixV(bounds.vector_to_matrix(vectorA));

  epetraBlockMatrixAssemble assembler(1,2);
  assembler.setEpetraComm(epetraComm);
  
  assembler.setRowMap(1,rowMapA);
  assembler.setColMap(1,colMapA);
  assembler.setColMap(2,matrixV.DomainMap());
  
  assembler.startup();
  
  assembler.setMatrix(matrixAA,1,1);
  assembler.setMatrix(matrixV,1,2);
  assembler.assemble();
