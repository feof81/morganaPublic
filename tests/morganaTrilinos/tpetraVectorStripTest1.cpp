...
int main(int argc, char *argv[])
{
  ...
  typedef tpetraBlockVectorAssemble::RCP_VECTOR RCP_VECTOR;
  ...
  //Stripper
  tpetraVectorStrip tpetraVectorStrip(2); 
  tpetraVectorStrip.setEpetraComm(teuchosComm);
  
  tpetraVectorStrip.setRowMap(1,rowMapA);
  tpetraVectorStrip.setRowMap(2,rowMapB);
  tpetraVectorStrip.startup();
  
  cout << "Part of the vector" << endl;
  RCP_VECTOR outVector = tpetraVectorStrip.getVectorRcp(globVector,1);
  
  auto out = Teuchos::getFancyOStream(Teuchos::rcpFromRef(std::cout));
  outVector->describe(*out,Teuchos::VERB_EXTREME);
}
