...
int main(int argc, char *argv[])
{
  ...
  assert(world.size() == 2);
  Teuchos::RCP<Teuchos::MpiComm<int> > teuchosComm = rcp(new Teuchos::MpiComm<int> (world));
  ...
  RCP_MAP rowMapA(new MAP(globSize,teuchosRowA,0,teuchosComm));
  RCP_MAP rowMapB(new MAP(globSize,teuchosRowB,0,teuchosComm));
  RCP_MAP colMapA(new MAP(globSize,teuchosColA,0,teuchosComm));
  RCP_MAP colMapB(new MAP(globSize,teuchosColB,0,teuchosComm));
  
  
  //Matrix matrixAA------------------------------------------------------------
  RCP_MATRIX matrixAA = Teuchos::rcp(new MATRIX(rowMapA,3));
  
  TPETRA_GLOBAL_ORDINAL length = 3;
  TPETRA_SCALAR valuesAA[length];
  TPETRA_GLOBAL_ORDINAL  indicesAA[length];
  TPETRA_GLOBAL_ORDINAL  numEntriesAA, rowAA;
  ...
  //Matrix matrixAB------------------------------------------------------------
  RCP_MATRIX matrixAB = Teuchos::rcp(new MATRIX(rowMapA,colMapB,3));
  
  TPETRA_SCALAR         valuesAB[length];
  TPETRA_GLOBAL_ORDINAL numEntriesAB, rowAB, indicesAB[length];
  ...
  //Matrix matrixBA------------------------------------------------------------
  RCP_MATRIX matrixBA = Teuchos::rcp(new MATRIX(rowMapB,colMapA,3));
  
  TPETRA_SCALAR         valuesBA[length];
  TPETRA_GLOBAL_ORDINAL numEntriesBA, rowBA, indicesBA[length];
  ...
  //Matrix matrixBB------------------------------------------------------------
  RCP_MATRIX matrixBB = Teuchos::rcp(new MATRIX(rowMapB,colMapB,3));
  
  TPETRA_SCALAR         valuesBB[length];
  TPETRA_GLOBAL_ORDINAL numEntriesBB, rowBB, indicesBB[length];
