...
int main(int argc, char *argv[])
{
  ...
  if(world->rank() == 2)
  {
    item.setLid(1); item.setGid(4);  item.setOwned(false); item.setShared(false); sMap->push_back(item);
    ...
    item.setLid(7); item.setGid(10); item.setOwned(false); item.setShared(false); sMap->push_back(item);
  }
...
pid: 0 lid: 2 gid: 10 shared: 1 owned 1
...
/* Pid 2
pid: 2 lid: 1 gid: 4 shared: 1 owned 0
...
pid: 2 lid: 7 gid: 10 shared: 1 owned 0
*/
