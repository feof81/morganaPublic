...
int main(int argc, char *argv[])
{
  ...
  pVectGlobalManip<Real,pMapItem> vectorManip(world);
  
  if(world->rank() == 0)
  {
    item.setPid(0); item.setLid(1); item.setGid(3); sVector.push_back(item,3.0,false);
    item.setPid(0); item.setLid(2); item.setGid(4); sVector.push_back(item,4.0,false);
  }
  ...
  vectorManip.vectorNormalExclusive(sVector);
