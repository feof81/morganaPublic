...
int main(int argc, char *argv[])
{
  ...
  manipulator.exportEpetraMap(sMap,epetraMap,epetraComm,1); //Export to epetra
  manipulator.importEpetraMap(epetraMap,rMap);              //Import from epetra, the final map (rMap) must not change from the original one (sMap)
