...
int main(int argc, char *argv[])
{
  ...
  //! Search 3d------------------------------------------------------
  typedef geoOctTree<MESH3D,CONNECT3D> OCTTREE;
  
  OCTTREE octTree(grid3d,connectGrid3d);
  octTree.localInit(3.0);
  
  sVect<UInt> elements = octTree.getMatchingElementsExt(point3d(-0.1, 0.0, 0.0));
  cout << "elements number : " << elements.size() << endl;
  cout << "elements ratio  : " << Real(elements.size()) / Real(grid3d->getNumElements()) << endl;
}
