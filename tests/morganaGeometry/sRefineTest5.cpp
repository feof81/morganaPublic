...
int main(int argc, char *argv[])
{
  ...
  elList(1) = 1;
  elList(2) = 2;
  refiner.setRefinementParams(1.3, 1, elList);
  
  cout << refiner.getEdgesTbr(10) << endl;
  
  refiner.getEdgeTbr(edge);
  cout << "edge to refine: " << edge << endl;
  refiner.refineEdge(edge); 
  
  refiner.printElements();
  refiner.print();
}
