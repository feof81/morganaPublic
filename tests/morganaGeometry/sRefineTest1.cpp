...
int main(int argc, char *argv[])
{
  ...
  typedef std::set<EDGE,edgeComp>::iterator ITER;
  ...
  edge.setPoint(1,point3d(1.0, 0.0, 0.0));
  edge.setPoint(2,point3d(1.0, 1.0, 0.0));
  edge.reorder();
  edges.insert(edge);
  
  edge.setPoint(1,point3d(0.0, 0.0, 0.0));
  edge.setPoint(2,point3d(0.5, 0.0, 0.0));
  edge.reorder();
  edges.insert(edge);
  
  edge.setPoint(1,point3d(1.0, 1.0, 0.0));
  edge.setPoint(2,point3d(1.0, 1.0, 0.5));
  edge.reorder();
  edges.insert(edge);
  
  cout << "Printout  of the set" << endl;
  for(ITER iter = edges.begin(); iter != edges.end(); ++iter)
  { cout << *iter << endl; }
  ...
  refiner.nodes2d.push_back(point3d(0.5, 0.0, 0.0));
  
  refiner.splitElement2d(edge, 6, 1);
  refiner.splitElement2d(edge, 6, 2);
  
  cout << "printout of the refined mesh" << endl;
  refiner.print();
}
