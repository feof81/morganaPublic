...
int main(int argc, char *argv[])
{
  ...
  typedef feStaticField3dGlobalManip<FETYPE,DOFTYPE,dms3d_vectMajor,dms3d_geoIdMode> GLOBMANIP;
  typedef GLOBMANIP::PVPS PVPS;
  
  GLOBMANIP manipulator(world);
  ...
  manipulator.createSendRecvMap(field3d, mapSend, mapRecv);
  
  if(pid == 0)
  {
    cout << "Map send-recieve" << endl;
    cout << mapSend << endl << endl;
  }
  
  //Polluting the dofsVector
  pVect<DOFTYPE,PMAPTYPE> dofsVect = field3d.getDofVect();  
  point3d V(10.0, 10.0, 10.0);
  
  for(UInt i=1; i <= dofsVect.size(); ++i)
  {
    if( dofsVect.getMapL(i).getShared() && (!dofsVect.getMapL(i).getOwned()) )
    {
      dofsVect.getDataL(i) = V;
    }
  }
  field3d.setDofVect(dofsVect);
  
  //Testing the update functions -> restore the correct values
  typedef feStaticField3d<FETYPE,DOFTYPE,dms3d_vectMajor,dms3d_geoIdMode>::DOFVECT DOFVECT;
  
  sVect<DOFVECT> bufSegments;
  sVect<PVPS>    sendPvps, recvPvps;
  
  manipulator.updateDataI(field3d,
                          mapSend,
                          mapRecv,
                          bufSegments,
                          sendPvps,
  ...
  manipulator.updateDataO(field3d,
                          mapSend,
                          mapRecv,
                          bufSegments,
                          sendPvps,
