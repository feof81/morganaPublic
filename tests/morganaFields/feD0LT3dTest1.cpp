...
#include "feD0LT3d.hpp"

int main(int argc, char *argv[])
{
  typedef pMapItem                   PMAPTYPE;
  typedef feD0LT3d<PMAPTYPE>         FETYPE;
  ...
  nodes(3) = point3d(1.0, 0.0, 0.0);
  nodes(4) = point3d(1.0, 1.0, 1.0);
  ...
  sVect<BASETYPE> locEval(numBasis), locGradX(numBasis), locGradY(numBasis), locGradZ(numBasis);
  sVect<BASETYPE> globalEval(numBasis), globalGradX(numBasis), globalGradY(numBasis), globalGradZ(numBasis);
