...
int main(int argc, char *argv[])
{
  ...
  feStaticField3dGlobalManip<FETYPE,DOFTYPE,dms3d_vectMajor,dms3d_allMode> manipulator(world);
  ...
  sVect<string> symbs(1);
  symbs(1) = "a";
  
  sVect<Real> symbsVals(1);
  symbsVals(1) = 1.0;  
  
  manipulator.initilize(functions,symbs,symbsVals,field3d);
  
  //Eval max
  sVect<point3d> evalNodes(1);
  evalNodes(1) = point3d(0.25, 0.25, 0.25);
  
  set<UInt> activeIds;
  activeIds.insert(1);
  activeIds.insert(2);
  
  cout << "max: " << manipulator.evalMax(field3d,evalNodes,activeIds) << endl;
}
