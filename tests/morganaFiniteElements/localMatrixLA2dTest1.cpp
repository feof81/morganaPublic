...
int main(int argc, char *argv[])
{
  ...
  //Local Matrix
  typedef localMatrixLA2d<FIELD,TEST,intDefault,STANDARD,2> LOCMATRIX;
  LOCMATRIX locMatrix(operatore);
  ...
  if(pid == 1)
  {
    ...
    //Local matrix
    UInt numCol = locMatrix.numIndex_col();
    sVect<UInt> indicesCol(numCol);    
    locMatrix.indexG_col(indicesCol);
    
    UInt numRow = locMatrix.numIndex_row();
    sVect<UInt> indicesRow(numRow);    
    locMatrix.indexG_row(indicesRow);
    
    sVect<Real> mat = locMatrix.matrix();
    
    cout << "Indices Row : " << endl << indicesRow << endl;
    cout << "Indices Col : " << endl << indicesCol << endl;
