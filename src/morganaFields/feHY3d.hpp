...
class feHY3d
{
  ...
  public:
    ...
    static const FELabel     feLabel     = FE_HY_3d;
    static const FEBaseLabel feBaseLabel = BL_HY_3d;    
    //@}
  ...
  public:
    feHY3d();
    feHY3d(const FECARD & FECard, const ELCARD & ELCard);
...
template<typename PMAPTYPE>
sVect<typename feHY3d<PMAPTYPE>::DOFCARD>
feHY3d<PMAPTYPE>::
getDofCards() const
{
  assert(elCardLoaded);
...
{  
  ...
  if(FeCard.getActiveFaces()(1)) {val(k) = Real(act(1)); ++k;}
  if(FeCard.getActiveFaces()(2)) {val(k) = Real(act(2)); ++k;}
  if(FeCard.getActiveFaces()(3)) {val(k) = Real(act(3)); ++k;}
  if(FeCard.getActiveFaces()(4)) {val(k) = Real(act(4)); ++k;}
