...
#ifndef FEOSCHYB3D_HPP
#define FEOSCHYB3D_HPP
...
class feOscHYB3d
{
  ...
  public:
    ...
    static const FELabel     feLabel     = FE_OSCHYB_3d;
    static const FEBaseLabel feBaseLabel = BL_OSCHYB_3d;
    //@}
  ...
  public:
    feOscHYB3d();
    feOscHYB3d(const FECARD & FECard, const ELCARD & ELCard);
...
template<typename PMAPTYPE>
sVect<typename feOscHYB3d<PMAPTYPE>::DOFCARD>
feOscHYB3d<PMAPTYPE>::
getDofCards() const
{
  assert(elCardLoaded);
...
localEval(const point3d & Y, sVect<BASETYPE> & val) const
{
  ...
  {val(k) = komplex(Real(act(1)) * (Real(FeCard.getFaceOrientation(1) == true) - Real(FeCard.getFaceOrientation(1) == false)),0.0); ++k;}
  
  if(FeCard.getActiveFaces(2))
  {val(k) = komplex(Real(act(2)) * (Real(FeCard.getFaceOrientation(2) == true) - Real(FeCard.getFaceOrientation(2) == false)),0.0); ++k;}
  
  if(FeCard.getActiveFaces(3))
  {val(k) = komplex(Real(act(3)) * (Real(FeCard.getFaceOrientation(3) == true) - Real(FeCard.getFaceOrientation(3) == false)),0.0); ++k;}
  
  if(FeCard.getActiveFaces(4))
  {val(k) = komplex(Real(act(4)) * (Real(FeCard.getFaceOrientation(4) == true) - Real(FeCard.getFaceOrientation(4) == false)),0.0); ++k;}
...
localGrad(const point3d & Y, sVect<BASETYPE> & gradX, sVect<BASETYPE> & gradY, sVect<BASETYPE> & gradZ) const
{
  ...
  {
    gradX(i) = komplex(0.0,0.0);
    gradY(i) = komplex(0.0,0.0);
    gradZ(i) = komplex(0.0,0.0);
  }
}
