...
#ifndef PRINTMESH1DHDF5_HPP
#define PRINTMESH1DHDF5_HPP
...
class printMesh1dHDF5
{
  ...
  public:
    printMesh1dHDF5();
    printMesh1dHDF5(const Teuchos::RCP<communicator> & CommDev);
    printMesh1dHDF5(communicator & CommDev);
    void setCommunicator(const Teuchos::RCP<communicator> & CommDev);
    void setCommunicator(communicator & CommDev);
    EpetraExt::DistArray<int>    convertElements(const MESH1D & grid1d) const;
    EpetraExt::DistArray<double> convertNodes(const MESH1D & grid1d) const;
    void print(const string & s, const UInt & pid, const Teuchos::RCP<MESH1D> & grid1d) const;
    void print(const string & s, const UInt & pid, const MESH1D & grid1d) const;
    void printGeoIds(const string & s, const UInt & pid, const Teuchos::RCP<MESH1D> & grid1d) const;
    void printGeoIds(const string & s, const UInt & pid, const MESH1D & grid1d) const;
    //@}
};
...
//-------------------------------------------------------------------------------------------------
template<typename GEOSHAPE, typename PMAPTYPE>
printMesh1dHDF5<GEOSHAPE,PMAPTYPE>::
printMesh1dHDF5()
{
...
printMesh1dHDF5<GEOSHAPE,PMAPTYPE>::
convertElements(const MESH1D & grid1d) const
{
  ...
  typedef typename MESH1D::GRAPH1D      ELEMENTS;
  typedef typename MESH1D::GEOELEMENT1D GEOELEMENT1D;
  
  ELEMENTS elements_morgana = grid1d.getElements();                 //Extract the elements
  elements_morgana.pushToGlobal();
  
  pVectGlobalManip<GEOELEMENT1D,PMAPTYPE> elementsManip(commDev);   //Redistribute
...
printMesh1dHDF5<GEOSHAPE,PMAPTYPE>::
convertNodes(const MESH1D & grid1d) const
{
  assert(commDevLoaded);
  ...
  //Nodes conversion-------------------------------------------------------------------------------
  typedef typename MESH1D::NODESVECT NODES;
  
  NODES nodes_morgana = grid1d.getNodes();  //Remap the nodes
...
void
printMesh1dHDF5<GEOSHAPE,PMAPTYPE>::
print(const string & s, const UInt & pid, const Teuchos::RCP<MESH1D> & grid1d) const
{
  assert(commDevLoaded);
  ...
  EpetraExt::DistArray<int>    elements_epetra = convertElements(*grid1d);
  EpetraExt::DistArray<double> nodes_epetra    = convertNodes(*grid1d);
...
void
printMesh1dHDF5<GEOSHAPE,PMAPTYPE>::
print(const string & s, const UInt & pid, const MESH1D & grid1d) const
{
  assert(commDevLoaded);
...
void
printMesh1dHDF5<GEOSHAPE,PMAPTYPE>::
printGeoIds(const string & s, const UInt & pid, const Teuchos::RCP<MESH1D> & grid1d) const
{
  assert(commDevLoaded);
  ...
  sVect<OUTTYPE> tempVect(grid1d->getNumElements());
   
  for(UInt i=1; i <= grid1d->getNumElements(); ++i)
  { tempVect(i) = Real(grid1d->getElementL(i).getGeoId()); }
...
void
printMesh1dHDF5<GEOSHAPE,PMAPTYPE>::
printGeoIds(const string & s, const UInt & pid, const MESH1D & grid1d) const
{
  assert(commDevLoaded);
  typedef Real DOFTYPE; 
