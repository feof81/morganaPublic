...
#ifndef PRINTMESH2DHDF5_HPP
#define PRINTMESH2DHDF5_HPP
...
class printMesh2dHDF5
{
  ...
  public:
    printMesh2dHDF5();
    printMesh2dHDF5(const Teuchos::RCP<communicator> & CommDev);
    printMesh2dHDF5(communicator & CommDev);
    void setCommunicator(const Teuchos::RCP<communicator> & CommDev);
    void setCommunicator(communicator & CommDev);
    EpetraExt::DistArray<int>    convertElements(const MESH2D & grid2d) const;
    EpetraExt::DistArray<double> convertNodes(const MESH2D & grid2d) const;
    void print(const string & s, const UInt & pid, const Teuchos::RCP<MESH2D> & grid2d) const;
    void print(const string & s, const UInt & pid, const MESH2D & grid2d) const;
    void printGeoIds(const string & s, const UInt & pid, const Teuchos::RCP<MESH2D> & grid2d) const;
    void printGeoIds(const string & s, const UInt & pid, const MESH2D & grid2d) const;
    //@}
};
...
//-------------------------------------------------------------------------------------------------
template<typename GEOSHAPE, typename PMAPTYPE>
printMesh2dHDF5<GEOSHAPE,PMAPTYPE>::
printMesh2dHDF5()
{
...
printMesh2dHDF5<GEOSHAPE,PMAPTYPE>::
convertElements(const MESH2D & grid2d) const
{
  ...
  typedef typename MESH2D::GRAPH2D      ELEMENTS;
  typedef typename MESH2D::GEOELEMENT2D GEOELEMENT2D;
  
  ELEMENTS elements_morgana = grid2d.getElements();                 //Extract the elements
  elements_morgana.pushToGlobal();
  
  pVectGlobalManip<GEOELEMENT2D,PMAPTYPE> elementsManip(commDev);   //Redistribute
...
printMesh2dHDF5<GEOSHAPE,PMAPTYPE>::
convertNodes(const MESH2D & grid2d) const    //TODO: FIX HERE!!!
{
  assert(commDevLoaded);
  ...
  //Nodes conversion-------------------------------------------------------------------------------
  typedef typename MESH2D::NODESVECT NODES;
  
  NODES nodes_morgana = grid2d.getNodes();  //Remap the nodes
...
void
printMesh2dHDF5<GEOSHAPE,PMAPTYPE>::
print(const string & s, const UInt & pid, const Teuchos::RCP<MESH2D> & grid2d) const
{
  assert(commDevLoaded);
  ...
  EpetraExt::DistArray<int>    elements_epetra = convertElements(*grid2d);
  EpetraExt::DistArray<double> nodes_epetra    = convertNodes(*grid2d);
...
void
printMesh2dHDF5<GEOSHAPE,PMAPTYPE>::
print(const string & s, const UInt & pid, const MESH2D & grid2d) const
{
  assert(commDevLoaded);
...
void
printMesh2dHDF5<GEOSHAPE,PMAPTYPE>::
printGeoIds(const string & s, const UInt & pid, const Teuchos::RCP<MESH2D> & grid2d) const
{
  assert(commDevLoaded);
  ...
  sVect<OUTTYPE> tempVect(grid2d->getNumElements());
   
  for(UInt i=1; i <= grid2d->getNumElements(); ++i)
  { tempVect(i) = Real(grid2d->getElementL(i).getGeoId()); }
...
void
printMesh2dHDF5<GEOSHAPE,PMAPTYPE>::
printGeoIds(const string & s, const UInt & pid, const MESH2D & grid2d) const
{
  assert(commDevLoaded);
  typedef Real DOFTYPE; 
