...
#ifndef OPEL3DA_CX_HPP
#define OPEL3DA_CX_HPP
...
/*! Operator 3d : c * grad(u) * grad(v) complex version*/
template<typename FIELD, typename TEST, typename COEFF>
class opEL3dA_CX : public operatorEL3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL3dA_CX();
    opEL3dA_CX(const Teuchos::RCP<communicator> & CommDev,
               const Teuchos::RCP<MESH3D>       & Grid3d,
               const Teuchos::RCP<CONNECT3D>    & ConnedGrid3d);
    
    opEL3dA_CX(communicator & CommDev,
               MESH3D       & Grid3d,
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST, typename COEFF>
opEL3dA_CX<FIELD,TEST,COEFF>::
opEL3dA_CX() : operatorEL3d<FIELD,TEST>()
{
...
eval(const point3d & Y, sVect<komplex> & mat)
{
  ...
  for(UInt i=1; i <= OPERATOR::numIndex_field(); ++i)
  {
    ...
    {      
      mat(tot) = ( MULTIPLIER::multiply(gradX_field(i), gradX_test(j) * V) +
                   MULTIPLIER::multiply(gradY_field(i), gradY_test(j) * V) +
                   MULTIPLIER::multiply(gradZ_field(i), gradZ_test(j) * V) ) * coeffVal;
      ++tot;
    }
  }
}
...
eval(const point3d & Y, sVect<Real> & mat)
{
  ...
  for(UInt i=1; i <= OPERATOR::numIndex_field(); ++i)
  {
    for(UInt j=1; j <= OPERATOR::numIndex_test(); ++j)
    {      
      mat(tot) = komplex::cComp( MULTIPLIER::multiply(gradX_field(i), gradX_test(j) * V) +
                                 MULTIPLIER::multiply(gradY_field(i), gradY_test(j) * V) +
                                 MULTIPLIER::multiply(gradZ_field(i), gradZ_test(j) * V), OPERATOR::I_test ) * coeffVal;
      ++tot;
    }
  }
}
