...
class opLA3dB : public operatorLA3d<FIELD,TEST>
{
  /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opLA3dB();
    opLA3dB(const Teuchos::RCP<communicator> & CommDev,
...
template<typename FIELD, typename TEST>
opLA3dB<FIELD,TEST>::
opLA3dB() : operatorLA3d<FIELD,TEST>()
{
...
template<typename FIELD, typename TEST>
opLA3dB<FIELD,TEST>::
opLA3dB(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d, const Teuchos::RCP<MESH2D> & Grid2d, const Teuchos::RCP<CONNECT2D> & ConnedGrid2d) : operatorLA3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d,Grid2d,ConnedGrid2d)
{
  assert(staticAssert<traitsBasic<FIELD_OUTTYPE>::myType == typePoint3d>::returnValue);
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typePoint3d>::returnValue);
}
...
template<typename FIELD, typename TEST>
opLA3dB<FIELD,TEST>::
opLA3dB(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d, MESH2D & Grid2d, CONNECT2D & ConnedGrid2d) : operatorLA3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d,Grid2d,ConnedGrid2d)
{
  assert(staticAssert<traitsBasic<FIELD_OUTTYPE>::myType == typePoint3d>::returnValue);
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typePoint3d>::returnValue);
}
