...
class opEL3dL : public operatorEL3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL3dL();
    opEL3dL(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d);
    opEL3dL(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d);
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST, typename COEFF>
opEL3dL<FIELD,TEST,COEFF>::
opEL3dL() : operatorEL3d<FIELD,TEST>()
{
...
template<typename FIELD, typename TEST, typename COEFF>
opEL3dL<FIELD,TEST,COEFF>::
opEL3dL(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d) : operatorEL3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d)
{
  assert(staticAssert<traitsBasic<FIELD_OUTTYPE>::myType == typeReal>::returnValue);
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typeReal>::returnValue);
  assert(staticAssert<traitsBasic<COEFF_OUTTYPE>::myType == typeReal>::returnValue);
...
template<typename FIELD, typename TEST, typename COEFF>
opEL3dL<FIELD,TEST,COEFF>::
opEL3dL(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d) : operatorEL3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d)
{
  assert(staticAssert<traitsBasic<FIELD_OUTTYPE>::myType == typeReal>::returnValue);
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typeReal>::returnValue);
  assert(staticAssert<traitsBasic<COEFF_OUTTYPE>::myType == typeReal>::returnValue);
...
eval(const point3d & Y, sVect<Real> & mat)
{
  ...
  for(UInt i=1; i <= OPERATOR::numIndex_field(); ++i)
  {
    ...
    {
      mat(tot) = - (eval_field(i) * eval_test(j) * val);
      ++tot;
    }
  }
}
