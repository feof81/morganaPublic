...
#ifndef OPLA3DA_CX_HPP
#define OPLA3DA_CX_HPP
...
class opLA3dA_CX : public operatorLA3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opLA3dA_CX();
    opLA3dA_CX(const Teuchos::RCP<communicator> & CommDev,
    ...
    opLA3dA_CX(communicator & CommDev,
               MESH3D       & Grid3d,
...
template<typename FIELD, typename TEST>
opLA3dA_CX<FIELD,TEST>::
opLA3dA_CX() : operatorLA3d<FIELD,TEST>()
{
...
eval(const point3d & Yf, sVect<komplex> & mat)
{
  ...
  for(UInt j=1; j <= OPERATOR::numIndex_test(); ++j)
  {
    ...
    {  
      mat(tot) = eval_field(i) * eval_test(j) * V;
      ++tot;
    }
  }
}
