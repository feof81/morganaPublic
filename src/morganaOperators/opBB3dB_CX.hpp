...
#ifndef OPBB3DB_CX_HPP
#define OPBB3DB_CX_HPP
...
/*! Operator 3d : -i * coeff * u * v, complex version*/
template<typename FIELD, typename TEST>
class opBB3dB_CX : public operatorBB3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opBB3dB_CX();
    opBB3dB_CX(const Teuchos::RCP<communicator> & CommDev,
    ...
    opBB3dB_CX(communicator & CommDev,
            MESH3D       & Grid3d,
...
template<typename FIELD, typename TEST>
opBB3dB_CX<FIELD,TEST>::
opBB3dB_CX() : operatorBB3d<FIELD,TEST>()
{
...
template<typename FIELD, typename TEST>
opBB3dB_CX<FIELD,TEST>::
opBB3dB_CX(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d, MESH2D & Grid2d, CONNECT2D & ConnedGrid2d) 
: operatorBB3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d,Grid2d,ConnedGrid2d)
{
  assert(staticAssert<traitsBasic<TEST_DOFTYPE>::myType  == typeKomplex>::returnValue);
  assert(staticAssert<traitsBasic<FIELD_DOFTYPE>::myType == typeKomplex>::returnValue);
...
eval(const point3d & Yf, sVect<komplex> & mat)
{
  ...
  for(UInt j=1; j <= OPERATOR::numIndex_test(); ++j)
  {
    ...
    {  
      mat(tot) = MULTIPLIER::multiply(I * eval_field(i), V * eval_test(j)) * active * coeff;
      ++tot;
    }
  }
}
