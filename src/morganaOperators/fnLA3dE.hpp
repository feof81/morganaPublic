...
class fnLA3dE : public functionalLA3d<TEST,GEOSHAPE3D>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    fnLA3dE();
    fnLA3dE(const Teuchos::RCP<communicator> & CommDev,
...
//-------------------------------------------------------------------------------------------------
template<typename TEST, typename GEOSHAPE3D, typename COEFF>
fnLA3dE<TEST,GEOSHAPE3D,COEFF>::
fnLA3dE() : functionalLA3d<TEST,GEOSHAPE3D>()
{
...
eval(const point3d & Yf, sVect<Real> & mat)
{
  ...
  { mat(j) = point3d::dot(val,eval_field(j)) ; }
}
