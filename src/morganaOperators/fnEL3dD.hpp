...
/*! Functional error evaluation INT( (u-v)^2 ) */
template<typename TEST, typename COEFF>
class fnEL3dD : public functionalEL3d<TEST>
{
  /*! @name Typedefs */ //@{
  public:
  ...
  public:
    bool coeffOk;
    bool analyticFunctionOk;
    Teuchos::RCP<COEFF> coeff;
    analyticField<Real> analyticFunction;
    //@}
    
    /*! @name Functions */ //@{
  public:
    fnEL3dD();
    fnEL3dD(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d);
    fnEL3dD(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d);
    void setCoeff(const Teuchos::RCP<COEFF> & Coeff);
    void setCoeff(COEFF & Coeff);
    void setAnalyticFunction(const analyticField<Real> & AnalyticFunction);
...
fnEL3dD() : functionalEL3d<TEST>()
{
  ...
  coeffOk            = false;
  analyticFunctionOk = false;
}

template<typename TEST, typename COEFF>
fnEL3dD<TEST,COEFF>::
fnEL3dD(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d) : functionalEL3d<TEST>(CommDev,Grid3d,ConnedGrid3d)
{
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typeReal>::returnValue);
  assert(staticAssert<traitsBasic<COEFF_OUTTYPE>::myType == typeReal>::returnValue);
...
template<typename TEST, typename COEFF>
fnEL3dD<TEST,COEFF>::
fnEL3dD(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d) : functionalEL3d<TEST>(CommDev,Grid3d,ConnedGrid3d)
{
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typeReal>::returnValue);
  assert(staticAssert<traitsBasic<COEFF_OUTTYPE>::myType == typeReal>::returnValue);
...
void
fnEL3dD<TEST,COEFF>::
setAnalyticFunction(const analyticField<Real> & AnalyticFunction)
{
  analyticFunctionOk = true;
  analyticFunction = AnalyticFunction;
  analyticFunction.startup();
}
...
eval(const point3d & Y, sVect<Real> & mat)
{
  ...
  //Global position
  sVect<point3d> points = FUNCTIONAL::grid3d->getElementNodesL(FUNCTIONAL::el);
  point3d            Pg = FUNCTIONAL::grid3d->getPosition(points,Y);
  
  //Vectoring
  for(UInt j=1; j <= FUNCTIONAL::numIndex_test(); ++j)
  {
    mat(j) = eval_field(j) * pow(val - analyticFunction.eval(Pg), 2);
  }
}
