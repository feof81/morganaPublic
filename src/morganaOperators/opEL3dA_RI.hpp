...
#ifndef OPEL3DA_RI_HPP
#define OPEL3DA_RI_HPP
...
class opEL3dA_RI : public operatorEL3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL3dA_RI();
    opEL3dA_RI(const Teuchos::RCP<communicator> & CommDev,
               const Teuchos::RCP<MESH3D>       & Grid3d,
               const Teuchos::RCP<CONNECT3D>    & ConnedGrid3d);
    
    opEL3dA_RI(communicator & CommDev,
               MESH3D       & Grid3d,
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST, typename COEFF>
opEL3dA_RI<FIELD,TEST,COEFF>::
opEL3dA_RI() : operatorEL3d<FIELD,TEST>()
{
...
eval(const point3d & Y, sVect<Real> & mat)
{
  ...
  for(UInt i=1; i <= OPERATOR::numIndex_field(); ++i)
  {
    ...
    {
      mat(tot) = komplex::cReal( MULTIPLIER::multiply(Im * gradX_field(i), gradX_test(j)) +
