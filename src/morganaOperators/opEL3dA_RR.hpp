...
#ifndef OPEL3DA_RR_HPP
#define OPEL3DA_RR_HPP
...
class opEL3dA_RR : public operatorEL3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL3dA_RR();
    opEL3dA_RR(const Teuchos::RCP<communicator> & CommDev,
               const Teuchos::RCP<MESH3D>       & Grid3d,
               const Teuchos::RCP<CONNECT3D>    & ConnedGrid3d);
    
    opEL3dA_RR(communicator & CommDev,
               MESH3D       & Grid3d,
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST, typename COEFF>
opEL3dA_RR<FIELD,TEST,COEFF>::
opEL3dA_RR() : operatorEL3d<FIELD,TEST>()
{
...
eval(const point3d & Y, sVect<Real> & mat)
{
  ...
  for(UInt i=1; i <= OPERATOR::numIndex_field(); ++i)
  {
    ...
    {
      mat(tot) = komplex::cReal( MULTIPLIER::multiply(gradX_field(i),gradX_test(j)) +
