...
class opEL3dI : public operatorEL3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL3dI();
    opEL3dI(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d);
    opEL3dI(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d);
...
template<typename FIELD, typename TEST>
opEL3dI<FIELD,TEST>::
opEL3dI() : operatorEL3d<FIELD,TEST>()
{
...
template<typename FIELD, typename TEST>
opEL3dI<FIELD,TEST>::
opEL3dI(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d) : operatorEL3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d)
{
  assert(staticAssert<traitsBasic<FIELD_OUTTYPE>::myType == typePoint3d>::returnValue);
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typePoint3d>::returnValue);
}
...
template<typename FIELD, typename TEST>
opEL3dI<FIELD,TEST>::
opEL3dI(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d) : operatorEL3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d)
{
  assert(staticAssert<traitsBasic<FIELD_OUTTYPE>::myType == typePoint3d>::returnValue);
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typePoint3d>::returnValue);
}
