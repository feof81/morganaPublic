...
/*! Operator 2d : (U * N) * q / coeffV */
template<typename FIELD, typename TEST, typename COEFFV>
class opLA3dG : public operatorLA3d<FIELD,TEST>
{
  /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opLA3dG();
    opLA3dG(const Teuchos::RCP<communicator> & CommDev,
...
template<typename FIELD, typename TEST, typename COEFFV>
opLA3dG<FIELD,TEST,COEFFV>::
opLA3dG() : operatorLA3d<FIELD,TEST>()
{
...
{
  coeffOk = true;
  coeffV  = Teuchos::rcp(new COEFFV(CoeffV));
}
...
{    
  ...
  for(UInt j=1; j <= OPERATOR::numIndex_test(); ++j)
  {
    ...
    {
      mat(tot) = ( point3d::dot(eval_field(i), N) * eval_test(j) / valV );
      ++tot;
    }
  }
}
