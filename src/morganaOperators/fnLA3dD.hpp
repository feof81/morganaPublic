...
class fnLA3dD : public functionalLA3d<TEST,GEOSHAPE3D>
{
  ...
  public:
    fnLA3dD();
    fnLA3dD(const Teuchos::RCP<communicator> & CommDev,
...
//-------------------------------------------------------------------------------------------------
template<typename TEST, typename GEOSHAPE3D, typename COEFF>
fnLA3dD<TEST,GEOSHAPE3D,COEFF>::
fnLA3dD() : functionalLA3d<TEST,GEOSHAPE3D>()
{
  ...
  assert(staticAssert<traitsBasic<COEFF_OUTTYPE>::myType == typePoint2d>::returnValue);
...
eval(const point3d & Yf, sVect<Real> & mat)
{
  ...
  {
    mat(j) = val.getX() * eval_field(j).getX() +
             val.getY() * eval_field(j).getY();
  }
}
