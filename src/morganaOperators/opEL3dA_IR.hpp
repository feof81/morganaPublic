...
#ifndef OPEL3DA_IR_HPP
#define OPEL3DA_IR_HPP
...
/*! Operator 3d : c * grad(u) * grad(v) complex version [Real field, Real equation]*/
template<typename FIELD, typename TEST, typename COEFF>
class opEL3dA_IR : public operatorEL3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL3dA_IR();
    opEL3dA_IR(const Teuchos::RCP<communicator> & CommDev,
               const Teuchos::RCP<MESH3D>       & Grid3d,
               const Teuchos::RCP<CONNECT3D>    & ConnedGrid3d);
    
    opEL3dA_IR(communicator & CommDev,
               MESH3D       & Grid3d,
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST, typename COEFF>
opEL3dA_IR<FIELD,TEST,COEFF>::
opEL3dA_IR() : operatorEL3d<FIELD,TEST>()
{
...
{
  typedef operatorEL3d<FIELD,TEST>  OPERATOR;
  typedef traitsMultiply<FIELD_OUTTYPE,TEST_OUTTYPE> MULTIPLIER;
  ...
  for(UInt i=1; i <= OPERATOR::numIndex_field(); ++i)
  {
    ...
    {
      mat(tot) = komplex::cImag( MULTIPLIER::multiply(gradX_field(i),gradX_test(j)) +
                                 MULTIPLIER::multiply(gradY_field(i),gradY_test(j)) +
                                 MULTIPLIER::multiply(gradZ_field(i),gradZ_test(j)) ) * coeffVal;
      ++tot;
    }
  }
}
