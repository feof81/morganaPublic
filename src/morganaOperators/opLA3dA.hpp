...
class opLA3dA : public operatorLA3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opLA3dA();
    opLA3dA(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d, const Teuchos::RCP<MESH2D> & Grid2d, const Teuchos::RCP<CONNECT2D> & ConnedGrid2d);
    opLA3dA(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d, MESH2D & Grid2d, CONNECT2D & ConnedGrid2d);
...
template<typename FIELD, typename TEST>
opLA3dA<FIELD,TEST>::
opLA3dA() : operatorLA3d<FIELD,TEST>()
{
...
template<typename FIELD, typename TEST>
opLA3dA<FIELD,TEST>::
opLA3dA(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d, MESH2D & Grid2d, CONNECT2D & ConnedGrid2d) : operatorLA3d<FIELD,TEST>(CommDev,Grid3d,ConnedGrid3d,Grid2d,ConnedGrid2d)
{
  assert(staticAssert<traitsBasic<FIELD_OUTTYPE>::myType == typeReal>::returnValue);
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typeReal>::returnValue);
}
