...
#ifndef OPEL3DB_CX_HPP
#define OPEL3DB_CX_HPP
...
class opEL3dB_CX : public operatorEL3d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL3dB_CX();
    opEL3dB_CX(const Teuchos::RCP<communicator> & CommDev,
               const Teuchos::RCP<MESH3D>       & Grid3d,
               const Teuchos::RCP<CONNECT3D>    & ConnedGrid3d);
    
    opEL3dB_CX(communicator & CommDev,
               MESH3D       & Grid3d,
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST>
opEL3dB_CX<FIELD,TEST>::
opEL3dB_CX() : operatorEL3d<FIELD,TEST>()
{
  ...
  for(UInt i=1; i <= OPERATOR::numIndex_field(); ++i)
  {
    ...
    {      
      mat(tot) = ( MULTIPLIER::multiply(gradX_field(i), gradX_test(j) * V) +
                   ...
                   MULTIPLIER::multiply(gradZ_field(i), gradZ_test(j) * V) );
      ++tot;
    }
  }
}
