...
/*! Operator 2d : (U * V) with U, V general types */
template<typename FIELD, typename TEST>
class opEL2dF : public operatorEL2d<FIELD,TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opEL2dF();
    opEL2dF(const Teuchos::RCP<communicator> & CommDev,
            const Teuchos::RCP<MESH2D>       & Grid2d,
            const Teuchos::RCP<CONNECT2D>    & ConnedGrid2d);
    
    opEL2dF(communicator & CommDev,
            MESH2D       & Grid2d,
...
template<typename FIELD, typename TEST>
opEL2dF<FIELD,TEST>::
opEL2dF() : operatorEL2d<FIELD,TEST>()
{
