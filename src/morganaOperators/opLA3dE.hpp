...
/*! Operator 2d : (U * Q) * (coeffH / coeffV) */
template<typename FIELD, typename TEST, typename COEFFV, typename COEFFH>
class opLA3dE : public operatorLA3d<FIELD,TEST>
{
  /*! @name Typedefs */ //@{
  public:
  ...
  public:
    opLA3dE();
    opLA3dE(const Teuchos::RCP<communicator> & CommDev,
...
template<typename FIELD, typename TEST, typename COEFFV, typename COEFFH>
opLA3dE<FIELD,TEST,COEFFV,COEFFH>::
opLA3dE() : operatorLA3d<FIELD,TEST>()
{
...
eval(const point3d & Yf, sVect<Real> & mat)
{
  ...
  coeffH->evalL(OPERATOR::getEl3d(),Yv,valH);
  ...
  for(UInt j=1; j <= OPERATOR::numIndex_test(); ++j)
  {
    ...
    {
      mat(tot) = ( point3d::dot(eval_field(i), eval_test(j)) * (valH / valV) );
      ++tot;
    }
  }
}
