...
class fnEL3dG : public functionalEL3d<TEST>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    fnEL3dG();
    fnEL3dG(const Teuchos::RCP<communicator> & CommDev,
            const Teuchos::RCP<MESH3D>       & Grid3d,
            const Teuchos::RCP<CONNECT3D>    & ConnedGrid3d);
    
    fnEL3dG(communicator & CommDev,
            MESH3D       & Grid3d,
...
fnEL3dG(const Teuchos::RCP<communicator> & CommDev,
        const Teuchos::RCP<MESH3D>       & Grid3d,
        const Teuchos::RCP<CONNECT3D>    & ConnedGrid3d)
: functionalEL3d<TEST>(CommDev,Grid3d,ConnedGrid3d)
{
  assert(staticAssert<traitsBasic<TEST_OUTTYPE>::myType  == typePoint2d>::returnValue);
  assert(staticAssert<traitsBasic<COEFF_OUTTYPE>::myType == typePoint2d>::returnValue);
...
eval(const point3d & Y, sVect<Real> & mat)
{
  ...
  {
    mat(j) = eval_field(j).getX() * val.getX() +
             eval_field(j).getY() * val.getY();
  }
}
