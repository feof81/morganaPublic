...
/*! Blobal search function, 2d meshes */
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
class search2dA : public localSearch2dA<GEOSHAPE,ELMAP,NODEMAP>
{
    /*! @name Typedefs */ //@{
  public:
    typedef geoElement<GEOSHAPE>            GEOELEMENT2D;
  ...
  public:
    search2dA();
    search2dA(const Teuchos::RCP<const communicator> & CommDev);
    search2dA(const communicator & CommDev);
...
//-------------------------------------------------------------------------------------------------
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
search2dA<GEOSHAPE,ELMAP,NODEMAP>::
search2dA() : localSearch2dA<GEOSHAPE,ELMAP,NODEMAP>::localSearch2dA()
{
...
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
search2dA<GEOSHAPE,ELMAP,NODEMAP>::
search2dA(const Teuchos::RCP<const communicator> & CommDev) : localSearch2dA<GEOSHAPE,ELMAP,NODEMAP>::localSearch2dA()
{
  assert(staticAssert<GEOSHAPE::nDim == 2>::returnValue);
...
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
search2dA<GEOSHAPE,ELMAP,NODEMAP>::
search2dA(const communicator & CommDev) : localSearch2dA<GEOSHAPE,ELMAP,NODEMAP>::localSearch2dA()
{
  assert(staticAssert<GEOSHAPE::nDim == 2>::returnValue);
...
globalInit()
{
  typedef localSearch2dA<GEOSHAPE,ELMAP,NODEMAP> LOCALSEARCH;
  ...
  all_gather(*commDev, LOCALSEARCH::grid2d->getNumElements(), globalNumElements);
...
findGlobal(pVect<point3d,NODEMAP> & Pvect, pVect<SEARCHDATA,NODEMAP> & DataVect)
{
  ...
  {
    DataVect(i) = localSearch2dA<GEOSHAPE,ELMAP,NODEMAP>::findLocal(Pvect(i));
  }
  ...
  {    
    dataOther(i) = localSearch2dA<GEOSHAPE,ELMAP,NODEMAP>::findLocal(Pother(i));
    
    el = dataOther(i).getElMap().getLid();
    if(dataOther(i).getFound()) { assert(LOCALSEARCH::grid2d->getElements().getRowMapL(el).getPid() == pid); }
  }
