...
#ifndef SEARCH3DA_HPP
#define SEARCH3DA_HPP
...
/*! Blobal search function, 3d meshes */
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
class search3dA : public localSearch3dA<GEOSHAPE,ELMAP,NODEMAP>
{
    /*! @name Typedefs */ //@{
  public:
  ...
  public:
    search3dA();
    search3dA(const Teuchos::RCP<const communicator> & CommDev);
    search3dA(const communicator & CommDev);
...
//-------------------------------------------------------------------------------------------------
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
search3dA<GEOSHAPE,ELMAP,NODEMAP>::
search3dA() : localSearch3dA<GEOSHAPE,ELMAP,NODEMAP>::localSearch3dA()
{
...
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
search3dA<GEOSHAPE,ELMAP,NODEMAP>::
search3dA(const Teuchos::RCP<const communicator> & CommDev) : localSearch3dA<GEOSHAPE,ELMAP,NODEMAP>::localSearch3dA()
{
 assert(staticAssert<GEOSHAPE::nDim == 3>::returnValue);
...
template<typename GEOSHAPE, typename ELMAP, typename NODEMAP>
search3dA<GEOSHAPE,ELMAP,NODEMAP>::
search3dA(const communicator & CommDev) : localSearch3dA<GEOSHAPE,ELMAP,NODEMAP>::localSearch3dA()
{
  assert(staticAssert<GEOSHAPE::nDim == 3>::returnValue);
...
globalInit()
{
  typedef localSearch3dA<GEOSHAPE,ELMAP,NODEMAP> LOCALSEARCH;
  ...
  all_gather(*commDev, LOCALSEARCH::grid3d->getNumElements(), globalNumElements);
...
findGlobal(pVect<point3d,NODEMAP> & Pvect, pVect<SEARCHDATA,NODEMAP> & DataVect)
{
  ...
  {
    DataVect(i) = localSearch3dA<GEOSHAPE,ELMAP,NODEMAP>::findLocal(Pvect(i));
  }
  ...
  {    
    dataOther(i) = localSearch3dA<GEOSHAPE,ELMAP,NODEMAP>::findLocal(Pother(i));
    
    el = dataOther(i).getElMap().getLid();
    if(dataOther(i).getFound()) { assert(LOCALSEARCH::grid3d->getElements().getRowMapL(el).getPid() == pid); }
  }
