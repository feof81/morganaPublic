...
#ifndef OPERATORBB2D_HPP
#define OPERATORBB2D_HPP
...
class operatorBB2d
{
  ...
  public:
    ...
    typedef mesh1d<GEOSHAPE1D,FIELD_PMAPTYPE,FIELD_PMAPTYPE>     MESH1D;
    typedef connect1d<GEOSHAPE1D,FIELD_PMAPTYPE,FIELD_PMAPTYPE>  CONNECT1D;
    ...
    typedef elCardFeeder1d<GEOSHAPE1D,PMAPTYPE>     ELCARDFEEDER;
  ...
  public:
    operatorBB2d();
    operatorBB2d(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH2D> & Grid2d, const Teuchos::RCP<CONNECT2D> & ConnectGrid2d, const Teuchos::RCP<MESH1D> & Grid1d, const Teuchos::RCP<CONNECT1D> & ConnectGrid1d);
    operatorBB2d(communicator & CommDev, MESH2D & Grid2d, CONNECT2D & ConnectGrid2d, MESH1D & Grid1d, CONNECT1D & ConnectGrid1d);
    virtual ~operatorBB2d() {};
...
template<typename FIELD, typename TEST>
const Teuchos::RCP<typename operatorBB2d<FIELD,TEST>::MESH2D> &
operatorBB2d<FIELD,TEST>::
getLoopGrid() const
{
  return(grid2d);
...
template<typename FIELD, typename TEST>
const typename operatorBB2d<FIELD,TEST>::FIELD_LISTMAP &
operatorBB2d<FIELD,TEST>::
...
template<typename FIELD, typename TEST>
const typename operatorBB2d<FIELD,TEST>::TEST_LISTMAP &
operatorBB2d<FIELD,TEST>::
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST>
const typename operatorBB2d<FIELD,TEST>::FIELD_FECARD &
operatorBB2d<FIELD,TEST>::
getFeCardL_field()
{
  assert(startupOk);
...
template<typename FIELD, typename TEST>
const typename operatorBB2d<FIELD,TEST>::TEST_FECARD &
operatorBB2d<FIELD,TEST>::
getFeCardL_test()
{
  assert(startupOk);
...
template<typename FIELD, typename TEST>
typename operatorBB2d<FIELD,TEST>::ELCARD
operatorBB2d<FIELD,TEST>::
getElCard() const
{
  assert(startupOk);
...
template<typename FIELD, typename TEST>
const Teuchos::RCP<typename operatorBB2d<FIELD,TEST>::INTGRID> &
operatorBB2d<FIELD,TEST>::
...
{
  assert(startupOk);
  fieldInterface.feEvalL(el2d, I_field, J_field, supportLagrange.mapVolumeY(Yf), val);
}
...
{
  assert(startupOk);
  fieldInterface.feGradL(el2d, I_field, J_field, supportLagrange.mapVolumeY(Yf), gradX, gradY, gradZ);
}
...
{
  assert(startupOk);
  testInterface.feEvalL(el2d, I_test, J_test, supportLagrange.mapVolumeY(Yf), val);
}
...
{
  assert(startupOk);
  testInterface.feGradL(el2d, I_test, J_test, supportLagrange.mapVolumeY(Yf), gradX, gradY, gradZ);
}
