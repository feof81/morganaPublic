...
#ifndef LOCALMATRIXLA3D_HPP
#define LOCALMATRIXLA3D_HPP

#include "morganaIntegrator.hpp"
#include "intTableLA3d.hpp"
...
class localMatrixLA3d
{
  ...
  public:
    static const intClass DEFAULTCLASS = intTableLA3d< FIELD_FETYPE::feBaseLabel, TEST_FETYPE::feBaseLabel >::intFlag;
  ...
  public:
    localMatrixLA3d();
    localMatrixLA3d(const Teuchos::RCP<OPERATOR> & Op);
  ...
  public:
    ...
    /*! @name Matrix building - single face */ //@{
  public:
    UInt locFace_numIndex_row(const UInt & f);
    void locFace_indexL_row(sVect<UInt> & indices, const UInt & f, const UInt & base = 0);
    void locFace_indexG_row(sVect<UInt> & indices, const UInt & f, const UInt & base = 0);
    
    sVect<Real> locFace_matrix(const UInt & f);
    //@}
};
...
localMatrixLA3d<FIELD,TEST,INT_CLASS,INT_TYPE,INT_PRECISION>::
localMatrixLA3d()
{
  assert(staticAssert<INTEGRATOR::opFlag == opLA3d>::returnValue);
  opLoaded = false;
}
...
//_________________________________________________________________________________________________
// MATRIX BUILD - RESTRICTED ON LOCAL FACE
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST, intClass INT_CLASS, intTypes INT_TYPE, UInt INT_PRECISION>
UInt
localMatrixLA3d<FIELD,TEST,INT_CLASS,INT_TYPE,INT_PRECISION>::
locFace_numIndex_row(const UInt & f)
{
  assert(opLoaded);
  UInt num = 0;
...
void
localMatrixLA3d<FIELD,TEST,INT_CLASS,INT_TYPE,INT_PRECISION>::
locFace_indexL_row(sVect<UInt> & indices, const UInt & f, const UInt & base)
{
  assert(opLoaded);
  assert(indices.size() == locFace_numIndex_row());
...
void
localMatrixLA3d<FIELD,TEST,INT_CLASS,INT_TYPE,INT_PRECISION>::
locFace_indexG_row(sVect<UInt> & indices, const UInt & f, const UInt & base)
{
  assert(opLoaded);
  assert(indices.size() == locFace_numIndex_row(f));
...
locFace_matrix(const UInt & f)
{
  ...
  UInt el3d = op->getEl3d();
  sVect<Real> tempMat;
  sVect<Real> mat(locFace_numIndex_row(f) * op->numIndex_field());
