...
#ifndef OPERATOREL3D_HPP
#define OPERATOREL3D_HPP

#include "elCardFeeder3d.hpp"
#include "morganaFiniteElements.hpp"


/*! Operarator for standard integration 3d
<b> Scheme </b>
<ol>
<li> field 3d
<li> test  3d
...
class operatorEL3d
{
  ...
  public:
    ...
    typedef elCard3d<FIELD_GEOSHAPE,FIELD_PMAPTYPE>       ELCARD;
    typedef elCardFeeder3d<FIELD_GEOSHAPE,FIELD_PMAPTYPE> ELCARDFEEDER;
  ...
  public:
    operatorEL3d();
    operatorEL3d(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d);
    operatorEL3d(communicator & CommDev, MESH3D & Grid3d, CONNECT3D & ConnedGrid3d);
    virtual ~operatorEL3d() {};
...
operatorEL3d(const Teuchos::RCP<communicator> & CommDev, const Teuchos::RCP<MESH3D> & Grid3d, const Teuchos::RCP<CONNECT3D> & ConnedGrid3d)
{
  ...
  fieldInterface.setGeometry(Grid3d,ConnedGrid3d);
...
template<typename FIELD, typename TEST>
const Teuchos::RCP<typename operatorEL3d<FIELD,TEST>::MESH3D> &
operatorEL3d<FIELD,TEST>::
getLoopGrid() const
{
  return(grid3d);
...
template<typename FIELD, typename TEST>
const typename operatorEL3d<FIELD,TEST>::FIELD_LISTMAP &
operatorEL3d<FIELD,TEST>::
...
template<typename FIELD, typename TEST>
const typename operatorEL3d<FIELD,TEST>::TEST_LISTMAP &
operatorEL3d<FIELD,TEST>::
...
//-------------------------------------------------------------------------------------------------
template<typename FIELD, typename TEST>
const typename operatorEL3d<FIELD,TEST>::FIELD_FECARD &
operatorEL3d<FIELD,TEST>::
getFeCardL_field()
{
  assert(startupOk);
...
template<typename FIELD, typename TEST>
const typename operatorEL3d<FIELD,TEST>::TEST_FECARD  &
operatorEL3d<FIELD,TEST>::
getFeCardL_test()
{
  assert(startupOk);
...
template<typename FIELD, typename TEST>
typename operatorEL3d<FIELD,TEST>::ELCARD
operatorEL3d<FIELD,TEST>::
getElCard() const
{
  assert(startupOk);
